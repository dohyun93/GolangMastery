...
// 인터페이스 타입 변환
// 1. 인터페이스의 메서드를 포함하지 않는 경우 인터페이스->해당 구조체로 타입 변경 불가하다 (컴파일에러).
// 2. A 타입을 가리키는 인터페이스를 B 타입으로 변환시 런타임에러가 발생한다. (A, B 모두 인터페이스의 메서드 집합을 포함한다고 가정.)
// 3. 다른 인터페이스로 타입 변환 시 변경 후 인터페이스가 전 인터페이스를 포함하지 않아도 된다. 그러나,
// 변환 전 인터페이스가 가리키던 인스턴스가 변환하고자 하는 인터페이스의 메서드를 포함해야 한다.
